    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Consumer<ApiProvider>(builder: (ctx, apiProvider, _) {
              return FutureBuilder<List<ResultData>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    return const Text('Personal ranking not available.');
                  } else {
                    return UserRankListTile(
              ...
              child: Consumer<ApiProvider>(
                builder: (ctx, apiProvider, _) {
                  return FutureBuilder<List<RankingData>>(
                    future: apiProvider.fetchRanking(testId),
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        return const Center(child: CircularProgressIndicator());
                      } else if (snapshot.hasError) {
                        ...
                        return const Text('Ranking data not available.');
                      } else {
                        return Scrollbar(
                  ...
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      ...
                      return ListView.builder(
                        shrinkWrap:
                            true, // Important to prevent the nested ListView from taking full height
